	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if((id!=null && id.equals("")) || (pw!=null && pw.equals(""))) {
			...
			if(dto2.getId() != null) {
				...
				writer.print("<script>alert('로그인 성공!');location.href='http://localhost:8081/project1/view.jsp'</script>");
				writer.close();
			}
			else {
				System.out.println("로그인 실패");
				writer.print("<script>alert('회원정보가 없습니다');location.href='http://localhost:8081/project1/view.jsp'</script>");
				writer.close();
